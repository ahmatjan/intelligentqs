...
public class TagsInfoDaoImp implements TagsInfoDaoInf{
	...
	public List<TagsInfoBean> getHotTags() {
		   List<TagsInfoBean> listTagBeans = new ArrayList<TagsInfoBean>();
			...
			PreparedStatement pst = null;
			ResultSet rs = null;
			String sql= "select * from tags_info limit 40,10";
			try {
				pst = con.prepareStatement(sql);
				rs = pst.executeQuery();
				while (rs.next()) {
	...
	//get TagsInfo by tags_categories_id
	public List<TagsInfoBean> getTagsInfoByCategory(int tags_categories_id){
		List<TagsInfoBean> listTagBeans = new ArrayList<TagsInfoBean>();
		TagsInfoBean tagBean = null;
		Connection con = db.getConn();
		PreparedStatement pst = null;
		ResultSet rs = null;
		String sql= "select * from tags_info where tags_categories_id = ?";
		try {
			pst = con.prepareStatement(sql);
